...
function moveNPC(npc) {
  ...
  if (walkingPath) {
    ...
    pathArray.unshift({
      'top': npc.style.top.replace('px', ''),
      'left': npc.style.left.replace('px', '')
    });
...
const hurtTheEnemy = function () {
  ...
  return function (theWeapon, value) {
    if (value && theWeapon && elementsOverlap(theWeapon.getBoundingClientRect(), value.getBoundingClientRect())) {
      if (called === false) {
...
function cleanClassName(classes) {
  return classes.replace('wp-block-group map-item ', '').replace('-map-item', '').replace('wp-block-group enemy-item ', '').replace(' no-point', '').replace(' is-layout-flow', '').replace(' wp-block-group-is-layout-flow', '').replace(' engage', '').replace('wp-block-group map-cutscene ', '').replace('-map-cutscene', '').replace('been-viewed', '').replace(' path-onload', '');
}
...
function blockMovement(top, left) {
  ...
  const collisionWalls = document.querySelectorAll('.default-map svg rect, .map-item:not(.drag-dest):not([data-trigger="true"]):not(.currently-dragging), .enemy-item');
