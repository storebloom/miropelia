    ...
    if ( cutscene ) {
        ...
        if (cutscene.classList.contains( 'engage' ) && false === cutscene.classList.contains( 'been-viewed' )) {
            ...
            if ( true === areaCutscene ) {
                moveDialogueBox();
                window.areaCutsceneInterval = setInterval( () => {
                    //nextDialogue();
                }, 2000 );
            }
            ...
            function nextDialogue() {
                ...
                if ( nextP ) {
                    ...
                    // clear interval from auto play.
                    clearInterval( window.areaCutsceneInterval );
                }
            }
            ...
            function moveDialogueBox() {
                ...
                if ( true === areaCutscene ) {
                    const currentDialogue = cutscene.querySelector( 'p.engage' );
                    const dialogueCharClass = '.' + currentDialogue.className.replace(' engage', '').replace('engage ', '') + '-map-item';
...
console.log(dialogueCharClass);
console.log(currentDialogueChar.style.top);

                    if ( currentDialogueChar && cutscene ) {
                        const currentDialogueCharLeft = '.mc-map-item' !== dialogueCharClass ? currentDialogueChar.style.left.replace('px', '') : currentDialogueChar.style.left.replace('px', '') + 500;
                        const currentDialogueCharTop = '.mc-map-item' !== dialogueCharClass ? currentDialogueChar.style.top.replace('px', '') : currentDialogueChar.style.top.replace('px', '') + 500;

                        cutscene.style.position = 'absolute';
                        cutscene.style.display = 'table';
                        cutscene.style.width = '400px';
                        cutscene.style.maxHeight = 'unset';
                        cutscene.style.height = 'unset';
                        cutscene.style.left = ( currentDialogueCharLeft - 430 ) + 'px';
                        cutscene.style.top = ( currentDialogueCharTop - cutscene.querySelector('.wp-block-group').offsetHeight - 30 ) + 'px';
                    }
                }
            }
        }
