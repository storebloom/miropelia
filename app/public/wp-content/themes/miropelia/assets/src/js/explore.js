...
document.addEventListener("DOMContentLoaded", function(){
    ...
    if (miroElExists(explorePage)) {
        ...
        const crewMates = document.querySelectorAll( '#characters .character-list .character-item' );
        ...
        window.talkingVolume = talkingSettings.value / 90;
        ...
        talkingSettings.addEventListener("input", (event) => {
            window.talkingVolume = event.target.value / 90;
        });
...
function addCharacter( character ) {
    const filehref = `https://${wpThemeURL}/wp-json/orbemorder/v1/add-character/${character}/${currentUserId}`;
...
function saveSettings(music, sfx, talking) {
    const filehref = `https://${wpThemeURL}/wp-json/orbemorder/v1/save-settings/${currentUserId}/${music}/${sfx}/${talking}`;
                ...
                xhr.addEventListener( "load", function ( e ) {
                    ...
                    if ( defaultMap ) {
                        setTimeout(() => {
                            ...
                            if ( crewMates ) {
                                crewMates.forEach( crewMate => {
                                    const characterName = cleanClassName( crewMate.className );
...
function selectNewCharacter( character ) {
    ...
    const currentCharacter = document.getElementById( 'map-character-icon' );

    if ( charImage && currentCharacter ) {
        const oldCurrentChar = currentCharacter.src;

        currentCharacter.setAttribute( 'src', charImage.src );
        charImage.setAttribute( 'src', oldCurrentChar );
        character.dataset.charactername = 'graeme';
    }
}
...
function engageExploreGame() {
    ...
    if ( isAreaCutscene && container ) {
        engageCutscene(container.className.replace( 'container ', ''), true);
    }
...
function miroExplorePosition(v,a,b,d,x, $newest) {
    ...
    const modal = document.querySelectorAll( '.map-item:not(.drag-dest), .projectile, .enemy-item' );
    ...
    forEach(modal, function(index, value) {
        ...
        if ( value && box && elementsOverlap( box.getBoundingClientRect(), value.getBoundingClientRect() ) ) {
            ...
            if ( true === value.classList.contains( 'mission-trigger' ) ) {
                if ( value.dataset.triggee && '' !== value.dataset.triggee ) {
                    saveMission( value.dataset.triggee, value, document.querySelector('.' + position + '-mission-item') );
                }
        ...
        if ( weaponEl ) {
            if ( elementsOverlap( weaponEl.getBoundingClientRect(), value.getBoundingClientRect() ) ) {
                ...
                if ( 'true' === value.dataset.breakable && 'explore-sign' !== value.dataset.genre ) {
                    value.remove();
                } else {
 ...
 * cut scene logic
 */
function engageCutscene( position, areaCutscene = false ) {
    ...
    if ( cutscene ) {
        ...
        if ( false === cutscene.classList.contains( 'been-viewed' ) ) {
            ...
            // on load.
            const text = dialogues[0].innerText;
            dialogues[0].innerText = '';
            typeWriter(dialogues[0], text, 0);
        }
        ...
        function moveDialogueBox() {
            ...
            const dialogueChar = currentDialogue.className.replace(' engage', '').replace('engage ', '');
            ...
            if ( true === areaCutscene ) {
                if ( currentDialogueChar && cutscene ) {
                    ...
                    cutscene.style.top = ( currentDialogueCharTop - cutscene.querySelector('.wp-block-group').offsetHeight ) + 'px';
                }
            } else {
                ...
                // set new char image.
                const newCharImage = document.querySelector( '.cut-character[data-character="' + dialogueChar + '"]' );

                if ( newCharImage ) {
                    newCharImage.classList.add( 'engage' );
                }
            }
        }
        ...
        function typeWriter(element, text, i) {
            if (i < text.length) {
                ...
                window.nextDialogueTimeout = setTimeout( () => {
                    nextDialogue();
                }, text.length < 40 ? 2000 : text.length > 100 ? 3000 : text.length * 50 );
            }
        }
...
function blockMovement(top, left) {
    ...
    const collisionWalls = document.querySelectorAll(
        '.default-map svg rect, .map-item:not(.drag-dest):not([data-trigger="true"]):not(.currently-dragging), .enemy-item'
    );
            ...
            if ( dragmeitem && true === dragmeitem.classList.contains( 'currently-dragging' ) ) {
                ...
                if ( dragDest ) {
                    ...
                    if ( topOffset < 10 && leftOffset < 10 && false === dragDest.classList.contains( 'completed-mission' ) ) {
                        saveMission(dragDest.dataset.mission, document.querySelector( '.' + dragDest.dataset.mission + '-mission-item' ), dragDest.dataset.mission );
