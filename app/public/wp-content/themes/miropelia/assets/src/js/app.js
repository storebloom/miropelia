...
function engageExploreGame() {
    ...
    setInterval( function () {
        ...
        if ( ( leftValInt >= 1099 && leftValInt <= 1149 ) &&
            ( topValInt >= 1846 && topValInt <= 2300 )
        ) {
            leftVal = ( leftValInt - 5 ) + 'px';
        }

        // Thorn fence right side.
        if ( ( leftValInt >= 3020 && leftValInt <= 3070 ) &&
            ( topValInt >= 1938 && topValInt <= 2300 )
        ) {
            leftVal = ( leftValInt + 5 ) + 'px';
        }

        // Kill character if they hit these positions.
        if ( false === miroHasClass(document.querySelector('.container'), 'gillimank') &&
            ( leftValInt >= 3632 || leftValInt <= 450 || topValInt >= 3989 || topValInt === 0 )
        ) {
            playSplashSound();
