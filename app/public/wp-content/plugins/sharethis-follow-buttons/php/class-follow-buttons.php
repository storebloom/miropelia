...
class Follow_Buttons {
	...
	public function define_sharethis_menus() {
		...
		if ( empty( $GLOBALS['admin_page_hooks']['sharethis-general'] ) ) {
			...
			add_menu_page(
				...
				__( 'ShareThis', 'sharethis-follow-buttons' ),
				'manage_options',
			...
			$this->general_hook_suffix = add_submenu_page(
				$this->menu_slug . '-general',
				__( 'ShareThis General Settings', 'sharethis-follow-buttons' ),
				__( 'General Settings', 'sharethis-follow-buttons' ),
				'manage_options',
				$this->menu_slug . '-general',
	...
	public function enqueue_admin_assets( $hook_suffix ) {
		...
		if ( $hook_suffix === $this->hook_suffix ) {
			...
			wp_add_inline_script( "{$this->plugin->assets_prefix}-admin", sprintf( 'FollowButtons.boot( %s );',
	...
	public function property_setting() {
		...
		$error_message = '' === $credential ? '<div class="st-error"><strong>' . esc_html__( 'ERROR', 'sharethis-follow-buttons' ) . '</strong>: ' . esc_html__( 'Property ID is required.', 'sharethis-follow-buttons' ) . '</div>' : '';
	...
	private function random_gif() {
		if ( ! is_wp_error( wp_safe_remote_get( 'http://api.giphy.com/v1/gifs/random?api_key=dc6zaTOxFJmzC&rating=g' ) ) ) {
			...
			return esc_html__( 'Sorry we couldn\'t show you a funny gif.  Refresh if you can\'t live without it.', 'sharethis-follow-buttons' );
		}
	}
	...
	public function update_follow_buttons() {
		...
		if ( 'On' === $onoff ) {
			update_option( 'sharethis_follow', 'true' );
		} elseif ( 'Off' === $onoff ) {
			update_option( 'sharethis_follow', 'false' );
		}
	}
				...
				?>
					<div class="notice notice-success is-dismissible">
						<p>
							...
							printf( esc_html__( 'Successfully reset your follow button position display options!', 'sharethis-follow-buttons' ), esc_html( sanitize_text_field( wp_unslash( $_GET['reset'] ) ) ) ); // WPCS: CSRF ok. Input var ok.
							?>
						</p>
					</div>
			<?php
	...
	public function activation_inform_notice() {
		...
		$gen_url = '<a href="' . esc_url( admin_url( 'admin.php?page=sharethis-follow-buttons&nft' ) ) . '">configuration</a>';
		...
		if ( get_transient( 'st-follow-activation' ) ) {
			...
			<div class="updated notice is-dismissible">
				<p>
					...
					printf( esc_html__( 'Your ShareThis Follow Button plugin requires %1$s', 'sharethis-share-button' ), wp_kses_post( $gen_url ) );
					?>
					.
				</p>
			</div>
		...
		if ( 'sharethis_page_sharethis-follow-buttons' === $screen->base && get_transient( 'st-follow-connection' ) && ! isset( $_GET['nft'] ) ) { // WPCS: CSRF ok. input var ok.
			?>
			<div class="notice notice-success is-dismissible">
				<p>
					<?php
					// translators: The product type.
					printf( esc_html__( 'Congrats! You’ve activated Follow Buttons. Sit tight, they’ll appear on your site in just a few minutes!', 'sharethis-follow-buttons' ), esc_html( $product ) );
					?>
				</p>
			</div>
			<?php
	...
	public function set_follow_button_config() {
		...
		$profiles = array_map( 'sanitize_text_field', wp_unslash( $_POST['config']['profiles'] ) ); // WPCS: input var ok.
		$first = isset( $_POST['first'] ) && 'upgrade' !== $_POST['first'] ? false : true; // WPCS: input var okay.
		...
		if ( ! $type ) {
			$newconfig['inline-follow'] = $config;
			$config = $newconfig;
		} else {
		...
		if ( ! $first ) {
			...
			$current_config['inline-follow'] = array_map( 'sanitize_text_field', wp_unslash( $_POST['config'] ) ); // WPCS: input var ok.
			$current_config['inline-follow']['networks'] = $networks;
			$current_config['inline-follow']['profiles'] = $profiles;
			$config = $current_config;
		}

		// Make sure bool is "true" or "false".
		if ( isset( $config['inline-follow'] ) ) {
